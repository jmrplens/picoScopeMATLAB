...
% 
% The file, PS5000A_ID_RAPID_BLOCK_PLOT3D_EXAMPLE.M must be on your MATLAB
...
%     Oscilloscope using the underlying 'A' API library functions and plot
%     the data using a 3-dimensional plot.
%
...
for i = 1:numCaptures
    ...
    plot3(axes1, timeNs, i * uint32(ones(numSamples, 1)), chA(:, i));
    
end

title(axes1, 'Channel A');
...
for i = 1:numCaptures
    ...
    plot3(axes2, timeNs, i * uint32(ones(numSamples, 1)), chB(:, i));
    
end

title(axes2, 'Channel B');
xlabel(axes2, 'Time (ns)');
ylabel(axes2, 'Capture');
zlabel(axes2, 'Voltage (mV)')
hold(axes2, 'off');

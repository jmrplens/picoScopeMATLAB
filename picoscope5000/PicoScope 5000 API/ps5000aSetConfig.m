function [ps5000aStructs, ps5000aEnuminfo] = ps5000aSetConfig()
% PS5000ASETCONFIG Configures paths according to platforms and loads information from
% prototype files for PicoScope 5000 Series (A API) Oscilloscopes. 
%
% This function allows the information from the prototype files to be loaded
% into the workspace that the picotech_ps5000a_generic.mdd Instrument Driver 
% is accessed from.
%
% The folder that this file is located in must be added to the MATLAB path.
%
...
% Copyright: © 2019 Pico Technology Ltd. See LICENSE file for terms.	
    ...
    if (isempty(strfind(path, ps5000aConfigInfo.archPath)))
        ...
        try
            ...
            error('ps5000aSetConfig:OperatingSystemNotSupported', 'Operating system not supported - please contact support@picotech.com');
    ...
    ps5000aConfigInfo.macDriverPath = [ps5000aConfigInfo.pathStr,filesep,'maci64'];
    ...
    if (ismac())
        ...
        if(contains(getenv('DYLD_LIBRARY_PATH'), ps5000aConfigInfo.macDriverPath))
            ...
            warning('ps5000aSetConfig:LibraryPathNotFound','Locations of libraries not found in DYLD_LIBRARY_PATH');
        ...
        if (strcmp(ps5000aConfigInfo.archStr, 'win32') && exist('C:\Program Files (x86)\', 'dir') == 7)
            ...
            if (isempty(strfind(path, ps5000aConfigInfo.woW64DriverPath)))
                ...
                try 
                    ...
                    warning('ps5000aSetConfig:DirectoryNotFound', ['Folder C:\Program Files (x86)\Pico Technology\SDK\lib\ not found. '...
            ...
            if (isempty(strfind(path, ps5000aConfigInfo.winDriverPath)))
                ...
                try 
                    ...
                    warning('ps5000aSetConfig:DirectoryNotFound', ['Folder C:\Program Files\Pico Technology\SDK\lib\ not found. '...
                        'Please ensure that the location of the library files are on the MATLAB path.']);
    ...
    if (~any(strcmp(ps5000aConfigInfo.psTbxName, {ps5000aConfigInfo.v.Name})))
        ...
        warning('ps5000aSetConfig:PSTbxNotFound', 'PicoScope Support Toolbox not found, searching for folder.');
        ...
        if (isempty(ps5000aConfigInfo.psTbxFound))
            ...
            warning('ps5000aSetConfig:PSTbxDirNotFound', ['PicoScope Support Toolbox not found. ', ps5000aConfigInfo.psTbxNotFoundWarningMsg]);
    ...
    [~, ps5000aStructs, ps5000aEnuminfo, ~] = ps5000aConfigInfo.ps5000aMFile(); 
